...
<div id="hepsi">
	...
	<div id="yer">
		<a id="yero" href="../main/urunler.php">Ürünler</a> -> <a id="yero" href="kurutmamakinesi.php">Kurutma Makinesi</a>
	</div>
		<div id="bolum">
	<div id="birinci"></div>
	<div id="ikinci">
		<div id="kate">
			<div id="ya">Kurutma Makinesi</div>
		</div>
		<?php
		...
		<a href="kurutmamakinesi.php" style="background-color: #41b5d8; color:#284b65; padding: 5px; border-radius: 5px;"> GERİ GİT</a>
		</center>
	...
	if (isset($_GET['islem']) && $_GET['islem'] == "satis" ){
								 ...
								 setcookie('urun['.$id.']',$id, time() + 86400);
									echo '<script>location.href = "kurutmamakinesi.php";</script>';
				}
	...
	?>
		...
		$sorgu = mysql_query("SELECT COUNT(*) AS toplam FROM tbl_urunler WHERE `KATEGORI` = 'KURUTMA MAKINESI' ORDER BY `ID` DESC");
		...
		?>
		<?php
				$sql=mysql_query("SELECT * FROM tbl_urunler  WHERE `KATEGORI` = 'KURUTMA MAKINESI' ORDER BY `ID` DESC LIMIT " . $limit . ", " . $sayfada);
